...
# Clears out the uniqname and password fields if login attempt unsuccessful
def clear_uniqname():
...
# Allows user to login when prompted to access order info
def user_login():
...
# Validates the PO entered is a 10 digit string
user_po_entry = ''
def validate_po(user_po_entry):
    import re
    if not re.match("^[0-9]*$", user_po_entry):
        print("The PO# must contain only digits.")
        po_search(user_po_entry)
    elif len(user_po_entry) != 10:
        print("The PO# must be 10 digits long.")
        po_search(user_po_entry)
    else:
